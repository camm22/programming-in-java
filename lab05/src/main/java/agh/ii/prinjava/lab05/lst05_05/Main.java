 ...
 *     Iteration over this set takes time proportional to the sum of the HashSet instance's size
 ...
 *     the iteration order, which is the order in which elements were inserted into the set (insertion-order).
 ...
 *     the linked list, with one exception: Iteration over a LinkedHashSet takes time proportional
 *     to the size of the set, regardless of its capacity. Iteration over a HashSet is likely to be more expensive,
 *     taking time proportional to its capacity.
 *
 *     </li>
 *     <li>{@link TreeSet}: a {@link NavigableSet} implementation based on a {@link TreeMap}.
 *     The elements are ordered using their natural order, or by a {@link Comparator} provided at set creation time,
 *     depending on which constructor is used.
 *     This implementation provides guaranteed log(n) time cost for the basic operations (add, remove and contains).
 *     Note that the order maintained by a set (whether or not an explicit comparator is provided)
 *     must be consistent with {@code equals} if it is to correctly implement the Set interface.</li>
 * </ul>
 * <p>
...
public class Main {
    ...
    private static void demo6() {
        ...
        ts.add(new C0("Rose"));
        System.out.println("as a TreeSet: " + ts);
        System.out.println("Is C0(Rose) in ts? " + ts.contains(new C0("Rose"))); // it works for a tree set, why? -> because we give comparator to ts.
