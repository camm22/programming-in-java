 ...
 *     Iteration over collection views takes time proportional to the "capacity" of the HashMap instance
 ...
 *     This linked list defines the iteration order, which is normally the order in which keys were inserted
 ...
 *     The map is sorted according to the natural order of its keys, or by a {@link Comparator} provided
 *     at map creation time, depending on which constructor is used.
 *     This implementation provides guaranteed log(n) time cost for the containsKey, get, put and remove operations.
 *     Note that the order maintained by a tree map, like any sorted map, and whether or not an explicit comparator
 *     is provided, must be consistent with {@code equals} if this sorted map is to correctly implement the Map interface.
 *     This is so because the Map interface is defined in terms of the {@code equals} operation, but a sorted map performs
 *     all key comparisons using its compareTo (or compare) method.</li>
 * </ul>
 * <p>
 *  {@link EnumMap}: a specialized Map implementation for use with enum type keys. All the keys in an enum map
