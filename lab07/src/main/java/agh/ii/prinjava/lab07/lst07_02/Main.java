...
//        Stream<Integer> test = LIST1.stream().filter(n -> n % 2 == 0).filter(n -> n>5);
...
//        Stream<Integer> s4 = Stream.iterate(1, n -> n + 2); // 1, 3, 5, 7,...
//        s4.anyMatch(x -> x % 2 == 0); // does NOT stop (all elements are odd)
    }
